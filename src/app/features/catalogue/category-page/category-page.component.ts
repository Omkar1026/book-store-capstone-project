      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="text-sm text-gray-500 mb-1">
            ...
            <span class="text-gray-900 font-medium">{{ categoryName() || categorySlug() }}</span>
          </nav>
          <h1 class="text-2xl font-bold text-gray-900">{{ categoryName() || 'Category' }}</h1>
        </div>
...
export class CategoryPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.categoryService.getCategories().subscribe({
      next: cats => {
        this.filterCategories.set(cats.map(c => ({ id: c.id, name: c.name })));
